...
async function run(page, job, signal, logger) {
    ...
    try {
        ...
        // 5. Update sheet
        const customNote = job.outputValues?.Note || job.sheetRow?._outputValues?.Note || 'done';
        if (sheetRow?.Gmail) await updateSheetAfterVerification(sheetRow.Gmail, customNote);
...
async function updateSheetAfterVerification(gmail, noteVal = 'done') {
    try {
        const port = process.env.API_PORT || 3334;
        await fetch(`http://127.0.0.1:${port}/api/sheet/update-note-and-date`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ gmail, note: noteVal, dateRestore: getCurrentDateGMT7(), automationName: 'verify-phone-sheet' }),
        });
    } catch { }
}
