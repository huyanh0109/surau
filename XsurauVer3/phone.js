...
function getMaxColLetter(columns, defaultMax = 'E') {
...
function getPhoneSheetConfig() {
    ...
    if (fs.existsSync(settingsPath)) {
        try {
            ...
            const checkConfig = settings.automations?.['check-phone-verify'];
            const customTabName = checkConfig?.sources?.[0]?.tab || checkConfig?.tab;
            const customSheetId = checkConfig?.sources?.[0]?.sheetId;

            if (customSheetId) {
                const foundSheet = settings.sheets?.find(s => s.id === customSheetId || s.spreadsheetId === customSheetId);
                if (foundSheet && foundSheet.spreadsheetId) spreadsheetId = foundSheet.spreadsheetId;
            }

            const targetTabName = customTabName || 'RentPhone';
            const tab = settings.tabs?.find(t => t.name === targetTabName) || settings.tabs?.find(t => t.name === 'RentPhone') || settings.tabs?.[0];
            if (tab) {
                sheetName = tab.name;
                if (tab.columns) columns = { ...columns, ...tab.columns };
            }
        } catch (e) {
            console.error('[PhoneSheet] Lỗi đọc settings.json:', e.message);
        }
    }

    if (!spreadsheetId || !sheetName) throw new Error('Missing GOOGLE_SHEET_ID or sheetName');
    return { spreadsheetId, sheetName, columns };
}

// ============================================================
// READER — Đọc sheet RentPhone (Dùng chung RAM cache từ google-sheet.js)
// ============================================================
...
async function getAllPhoneRows() {
    try {
        const { sheetName, spreadsheetId } = getPhoneSheetConfig();
        const tabRows = await getRowsFromTab(sheetName || 'RentPhone', spreadsheetId);
        ...
        console.error('[PhoneSheet] Lỗi đọc danh sách SĐT:', e.message);
        return [];
    }
}
...
async function updateLastUse(rowIndex, dateTimeStr) {
    ...
    const { spreadsheetId, sheetName, columns } = getPhoneSheetConfig();
    await sheets.spreadsheets.values.update({
        spreadsheetId,
        range: `'${sheetName}'!${columns.LastUse || 'D'}${rowIndex}`,
...
class PhoneQueue {
    ...
    async load(daysSinceLastUse = 0, limit = 300) {
        ...
        return { success: true, total: this.phones.length, available: this.phones.length, message: `Loaded ${this.phones.length} phones` };
    }

    /** Lấy số tiếp theo cho profile (profile là string ID của Xsurau) — Y HỆT BẢN CỦ */
    getNext(profileId) {
