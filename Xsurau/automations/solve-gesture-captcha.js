...
async function getGestureFrameInfo(frame) {
    ...
    }).catch(err => ({ heading: '', notoGesture: null, notoUrl: '', detectionSource: 'error-' + err.message, hasStart: false, hasTryAgain: false, hasNext: false, isSuccess: false, bodyText: '', allUrls: [] }));
}
...
async function solveCaptchaProcess(currentPage, job, signal, log) {
    ...
    solve_loop: for (let loopAttempt = 1; loopAttempt <= 5 && !signal?.aborted; loopAttempt++) {
        ...
        for (let attempt = 0; attempt < 180 && !signal?.aborted; attempt++) {
            ...
            if (attempt % 4 === 0) {
                log(`[step1] h="${info.heading}" noto=${info.notoGesture||'-'} src=${info.detectionSource||'-'} start=${info.hasStart} next=${info.hasNext}`);
                if (info.bodyText) log(`   text: ${info.bodyText.replace(/\n/g, ' ').substring(0, 120)}`);
                if (!info.notoGesture && info.allUrls && info.allUrls.length > 0) {
                    log(`   URLs: ${JSON.stringify(info.allUrls)}`);
                }
            }
        ...
        try {
            for (let attempt = 0; attempt < 120 && !captchaCompleted && !signal?.aborted; attempt++) {
                ...
                if (attempt % 4 === 0) {
                    log(`[step2] noto=${info.notoGesture||'-'} src=${info.detectionSource||'-'} start=${info.hasStart} tryAgain=${info.hasTryAgain} heading="${info.heading}"`);
                    if (info.bodyText) log(`   text: ${info.bodyText.replace(/\n/g, ' ').substring(0, 120)}`);
                    if (!info.notoGesture && info.allUrls && info.allUrls.length > 0) {
                        log(`   URLs: ${JSON.stringify(info.allUrls)}`);
                    }
                ...
                // Reset ve buoc 1
                if (h.includes('something went wrong') || info.notoGesture === 'hand_open') {
                    log('Reset ve buoc 1 -> tiep tuc solve_loop...');
                    gestureFrame = gf;
                    resetDetected = true;
                    break;
                }
                ...
                if (successInHeading || info.isSuccess) {
                    log('"You can lower your hand now" detected -> Thanh cong!');
                    captchaCompleted = true;
                    finalStep2Gesture = step2Gesture;
                    // Cho them 3s de Google redirect
...
async function run(page, job, signal, logger, options = {}) {
    ...
    log(`Gesture Captcha Solver (watchMode=${watchMode})`);
    ...
    while (!signal?.aborted) {
        try {
            ...
            const url = currentPage.url();
            if (!url.includes('accounts.google.com') && !url.includes('challenge') && !url.includes('recaptcha')) {
                await sleep(2000).catch(() => {});
                continue;
            }

            attemptCount++;
            log(`Lan thu ${attemptCount}...`);
            ...
            if (solveRes.success) {
                log(`Thanh cong! (${solveRes.gesture}) Lan ${attemptCount}`);
                if (!watchMode) { oneShotSuccess = true; break; }
                await sleep(5000).catch(() => {});
                attemptCount = 0;
            } else {
                log(`That bai (${solveRes.error}). Thu lai sau 2s...`);
                if (!watchMode) break;
                await sleep(2000).catch(() => {});
            }
        } catch (err) {
            ...
            if (msg === 'STOPPED' || msg.includes('Target closed') || msg.includes('Session closed') || msg.includes('context destroyed')) break;
            log(`Loi: ${msg}. Thu lai 3s...`);
            await sleep(3000).catch(() => {});
        }
    }

    log('Da dung.');
    return { profileId, success: watchMode ? true : oneShotSuccess };
}

module.exports = {
    name: 'solve-gesture-captcha',
    description: 'Vuot Google Gesture CAPTCHA: Y4M + Chromium switch-file patch',
    icon: '🖐️',
    run,
